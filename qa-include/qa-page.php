...
*/
	{
		...
		// 
		// convert from id to category names
		$category_tag = null;
		if($lastcategoryid != null)
		{
			list($fullcategory, $categories)=qa_db_select_with_pending(
					qa_db_full_category_selectspec($lastcategoryid, true),
					qa_db_category_sub_selectspec($lastcategoryid)
			);
		
			$category_tag = $fullcategory['tags'];
		}
		...
		list($categoryid)=qa_db_select_with_pending(
				qa_db_slugs_to_category_id_selectspec($lastcategoryid) 
		);
			
		// 
		// modify url to achieve category navigation container
		if (qa_opt('nav_questions'))
			$qa_content['navigation']['main']['questions']=array(
				'url' => qa_path_html('questions',(qa_using_categories() && strlen($lastcategoryid)) ? array('qa_1' => $category_tag) : null),
		...
		if (qa_opt('nav_unanswered'))
			$qa_content['navigation']['main']['unanswered']=array(
				'url' => qa_path_html('unanswered', (qa_using_categories() && strlen($lastcategoryid)) ? array('qa_1' => $category_tag) : null),
		...
		if (qa_using_tags() && qa_opt('nav_tags'))
			$qa_content['navigation']['main']['tag']=array(
				'url' => qa_path_html('tags', (qa_using_categories() && strlen($lastcategoryid)) ? array('qa_1' => $category_tag) : null),
		...
		if (qa_opt('nav_users'))
			$qa_content['navigation']['main']['user']=array(
				'url' => qa_path_html('users',(qa_using_categories() && strlen($lastcategoryid)) ? array('qa_1' => $category_tag) : null),
