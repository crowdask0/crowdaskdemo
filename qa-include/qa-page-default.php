...
//	Prepare and return content for theme for Q&A listing page
	...
	$categories=qa_db_select_with_pending(
			QA_ALLOW_UNINDEXED_QUERIES ? qa_db_category_sub_selectspec(NULL) : null
	);
	

	//	Then, add categories entry portals
	$qa_content['q_list']['schools'] = '';
	
	 if(count($categories) != 0)
	 {
		 foreach ($categories as $category)
		 {
			 $logo = '<a class="openid_large_btn context-login action-login" title="{$title}" href="{$url}" style="background: #fff url({$logo}); background-size: 100% 100%;">{$text}</a>';
			
			 $vars = array(
			 '{$title}' => $category['title'],
			 '{$url}' => qa_path_html('questions/'.$category['tags'], $pathparams),
			 '{$text}' => '',
			 '{$logo}' => $category['logo'],
			 );
			
			 $qa_content['q_list']['schools'] .= strtr($logo, $vars);
		 }
	 }
	 
	 $qa_content['q_list']['schools'].= '<div class="qa-q-item-clear">';
	 
	 if($countslugs > 0)
	 	$qa_content['q_list']['schools'] = null;
	 
	 //
	 if (count($questions)) 
	 	$qa_content['q_list']['title'] = $sometitle;
	 else 
	 	$qa_content['q_list']['title'] = $nonetitle;
