...
*/
	...
	*/
    ...
    <input value="Search" class="qa-search-button" type="submit">
		...
		{
			//
			if(isset($this->content['q_list']['schools']))
			{
				$this->output('Select Your Schools');
				return;
			}
		...
		function main_parts($content)
		{
			...
			foreach ($content as $key => $part) {
				...
				if($hasForm == true && strpos($key, 'custom')===0)
					$this->output("<br></br>");
		...
		{
			//
			if(isset($part['schools']))
			{
				$this->output('<h2>'.$part['title'].'</h2>');
				return ;
			}
	...
	*/
		...
		{
			//
			if(isset($q_list['schools']))
			{
				$this->output($q_list['schools']);
				$this->output('<br>');
